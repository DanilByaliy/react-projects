class ToDo {
  ...
  constructor() {
    ...
    for (const selectorKey in this.selectors) {
      if (selectorKey === "root") continue;

      this[`${selectorKey}Element`] = this.rootElement.querySelector(
  ...
  getItemsFromLocalStorage() {
    ...
    try {
      ...
      console.error("Todo items pare error");
      return [];
    }
  }
  ...
  render() {
    ...
    const items = this.state.filteredElements ?? this.state.items;
    ...
    this.listElement.innerHTML = items
      .map(
        ...
        <li class="todo__item todo-item" data-js-todo-item>
          ...
          >
          <button
            class="todo-item__delete_button"
            type="button"
            aria-label="Delete"
            title="Delete"
    ...
    const isEmptyFilteredItems = this.state.filteredElements?.length === 0;
  ...
  filter() {
    ...
    this.state.filteredElements = this.state.items.filter(({ title }) => {
  ...
  resetFilter() {
    this.state.filteredElements = null;
  ...
  onNewTaskFormSubmit = (event) => {
    ...
    const newTaskTitle = this.newTaskInputElement.value;

    if (newTaskTitle.trim().length == 0) {
      return;
    }

    this.addItem(newTaskTitle);
    this.resetFilter();
